...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (exists)
        return res.badRequest("Doctor already exists with this email");
      const hashed = await bcrypt.hash(req.body.password, 12);
      const doc = await Doctor.created({
        ...req.body,
        password: hashed,
      ...
      res.serverError("Resgistration failed", [error.message]);
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (exists)
        return res.badRequest("Patient already exists with this email");
      const hashed = await bcrypt.hash(req.body.password, 12);
      const patient = await Patient.created({
        ...req.body,
        password: hashed,
